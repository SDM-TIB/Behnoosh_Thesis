...
def run_full_validation():
    try:
        ...
        kg_path = os.path.join('KG', kg_name, rdf_file)  # KG/LungCancer-OriginalKG/LungCancer-OriginalKG.nt
        ...
        # Debug: Check shapes.ttl content
        shapes_file = os.path.join(constraints_dir, 'shapes.ttl')
        if os.path.exists(shapes_file):
            with open(shapes_file, 'r') as f:
                shapes_content = f.read()
            print(f"Content of shapes.ttl:\n{shapes_content}")
        else:
            print(f"shapes.ttl not found at {shapes_file}")
